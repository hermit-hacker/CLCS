-- Host: localhost    Database: Justice
-- ------------------------------------------------------
...
--
-- Table structure for table `Courthouses`
-- 
/*!40101 SET @saved_cs_client     = @@character_set_client */;
/*!40101 SET character_set_client = utf8 */;
CREATE TABLE `Courthouses` (
  `id` INT UNSIGNED NOT NULL AUTO_INCREMENT COMMENT 'UID for each Courthouse',
  `address` VARCHAR(255) NOT NULL COMMENT 'The FQDN or IP address of the Courthouse',
  `port` INT UNSIGNED NOT NULL COMMENT 'The port that Courthouse is listening on',
  `username` VARCHAR(255) COMMENT 'A username for traditional login',
  `userpass` VARCHAR(255) COMMENT 'A password for traditional login',
  `public_key` VARCHAR(5000) COMMENT 'The public key for the Courthouse',
  PRIMARY KEY (`id`)
) ENGINE=MyISAM DEFAULT CHARSET=latin1 COMMENT='Table of Courthouses';
/*!40101 SET character_set_client = @saved_cs_client */;
